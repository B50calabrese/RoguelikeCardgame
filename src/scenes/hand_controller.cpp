...
HandController::HandController() : hovered_card_index_(std::nullopt), held_card_index_(std::nullopt) {}

void HandController::Update(float delta_time_seconds, core::state::GameState& state) {
  if (core::effects::VisualBlocker::Get().IsBlocking()) {
    AnimateCards(delta_time_seconds);
    return;
  }
  SyncHandWithState(state.player->hand);
  HandleInteraction(state);
  UpdateLayout();
  AnimateCards(delta_time_seconds);
}

void HandController::SyncHandWithState(const std::vector<std::unique_ptr<core::CardInstance>>& hand_state) {
    auto& config = core::GameConfig::Get();

    // 1. Remove cards that are no longer in hand state
    hand_visuals_.erase(std::remove_if(hand_visuals_.begin(), hand_visuals_.end(), [&](const VisualCard& vc) {
        return std::none_of(hand_state.begin(), hand_state.end(), [&](const auto& inst) {
    ...
    for (const auto& inst : hand_state) {
        bool found = std::any_of(hand_visuals_.begin(), hand_visuals_.end(), [&](const VisualCard& vc) {
...
void HandController::HandleInteraction(core::state::GameState& state) {
  ...
  if (held_card_index_) {
    ...
    if (clicked) {
      ...
      if (inst) {
          for (const auto& effect_def : inst->data->effects) {
              if (effect_def.trigger == core::Trigger::OnPlay && effect_def.filter.is_required) {
                  ...
                  if (effect_def.filter.IsValid(state, 0, {core::effects::Target::Type::Enemy, 1})) {
                      targets.push_back({core::effects::Target::Type::Enemy, 1});
                  } else if (effect_def.filter.IsValid(state, 0, {core::effects::Target::Type::Player, 0})) {
      ...
      core::effects::EffectResolver::Get().QueueAction(
          std::make_shared<core::effects::actions::PlayCardAction>(0, held_card.instance_id, targets));
  ...
  for (int i = static_cast<int>(hand_visuals_.size()) - 1; i >= 0; --i) {
    ...
    glm::vec2 size = core::graphics::kBaseCardSize * vc.current_transform.scale.x;
    if (core::util::PointInRect(mouse_pos, vc.current_transform.position, size, true)) {
...
void HandController::AnimateCards(float delta_time_seconds) {
  ...
  for (auto& vc : hand_visuals_) {
    glm::vec2 old_pos = vc.current_transform.position;
    vc.current_transform.position = glm::mix(vc.current_transform.position, vc.target_transform.position, t);
    vc.current_transform.scale = glm::mix(vc.current_transform.scale, vc.target_transform.scale, t);
    vc.current_transform.rotation = glm::mix(vc.current_transform.rotation, vc.target_transform.rotation, t);

    if (glm::distance(vc.current_transform.position, vc.target_transform.position) > 1.0f) {
        any_moving = true;
    }
  }

  if (any_moving) {
      core::effects::VisualBlocker::Get().AddBlocker("HandAnimation");
  } else {
      core::effects::VisualBlocker::Get().RemoveBlocker("HandAnimation");
  }
}
