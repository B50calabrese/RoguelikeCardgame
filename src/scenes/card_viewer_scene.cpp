...
void CardViewerScene::OnAttach() {
  ...
  buttons_.push_back(std::make_unique<core::graphics::UIButton>(
      "Sort: Name",
      glm::vec2{kMargin, config.window_height - kBtnHeight - kMargin},
      glm::vec2{kBtnWidth, kBtnHeight}, [this]() {
  ...
  buttons_.push_back(std::make_unique<core::graphics::UIButton>(
      "Sort: Cost",
      glm::vec2{kMargin + kBtnWidth + kMargin,
                config.window_height - kBtnHeight - kMargin},
      glm::vec2{kBtnWidth, kBtnHeight}, [this]() {
  ...
  buttons_.push_back(std::make_unique<core::graphics::UIButton>(
      "Back",
      glm::vec2{config.window_width - kBtnWidth - kMargin,
                config.window_height - kBtnHeight - kMargin},
...
void CardViewerScene::HandleInput(float delta_time_seconds) {
  ...
  for (int i = 0; i < cards_.size(); ++i) {
    int row = i / cols;
    int col = i % cols;
    glm::vec2 pos = {start_x + col * (card_w + kCardPadding) + card_w * 0.5f,
                     start_y - row * (card_h + kCardPadding) - card_h * 0.5f};
...
void CardViewerScene::RenderUI() {
  ...
  float sb_y = 100.0f;
  float sb_height = config.window_height - 200.0f;
  renderer.DrawRect(sb_x, sb_y, kScrollbarWidth, sb_height, 0.2f, 0.2f, 0.2f);
  if (max_scroll_ > 0) {
    float handle_height =
    ...
    renderer.DrawRect(sb_x, handle_y, kScrollbarWidth, handle_height, 0.5f,
                      0.5f, 0.5f);
  }
