...
void NewRunScene::OnAttach() {
  ...
  float buffer = window_w * kBufferFactor;
  ...
  float char_y = window_h * kCharacterYFactor;
  ...
  float color_y = window_h * kColorYFactor;
...
void NewRunScene::OnRender() {
  ...
  for (int i = 0; i < static_cast<int>(characters_.size()); ++i) {
    ...
    renderer.DrawQuad(characters_[i].pos, kCharacterSize, color, 0.0f,
                      {0.5f, 0.5f});
    ...
    if (i == selected_character_index_) {
      ...
      glm::vec2 outline_size =
          kCharacterSize + glm::vec2(outline_thickness * 2.0f);
  ...
  for (int i = 0; i < static_cast<int>(colors_.size()); ++i) {
    ...
    if (selected) {
      ...
      float outline_thickness = 4.0f;
      glm::vec2 outline_size = kColorSize + glm::vec2(outline_thickness * 2.0f);
      renderer.DrawQuad(colors_[i].pos, outline_size, {1, 1, 1, 1}, 0.0f,
                        {0.5f, 0.5f});
    }

    renderer.DrawQuad(colors_[i].pos, kColorSize, color, 0.0f, {0.5f, 0.5f});
  }
