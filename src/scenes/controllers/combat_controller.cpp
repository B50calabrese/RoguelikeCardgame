    ...
    if (current_state_ == CombatState::Idle) {
      ...
      if (auto hitbox = hitbox_manager_.GetHitboxAt(mouse_pos)) {
        if (hitbox->is_enemy) {
          ...
          target.type = core::effects::Target::Type::kCreature;
          target.id = hitbox->instance_id;

          core::effects::EffectResolver::Get().QueueAction(
              std::make_shared<core::effects::actions::CreatureAttackAction>(
                  *selected_attacker_id_, target));
      ...
      if (!target_found) {
        ...
        if (core::util::PointInRect(mouse_pos, enemy_health_pos, icon_size_vec,
                                    true)) {
          core::effects::Target target;
          target.type = core::effects::Target::Type::kEnemy;
          target.id = -1;
...
void CombatController::OnCreatureAttacked(core::state::CombatState& state,
  ...
  if (event.target_id == -1) {
    if (state.current_turn_player_id == state.player->id) {
      target_pos = {config.window_width * 0.5f,
                    config.window_height - icon_top + icon_size * 0.5f};
      anim.target.type = core::effects::Target::Type::kEnemy;
    } else {
      target_pos = {config.window_width * 0.5f, icon_top - icon_size * 0.5f};
      anim.target.type = core::effects::Target::Type::kPlayer;
    }
  } else {
    anim.target.type = core::effects::Target::Type::kCreature;
    if (auto hitbox = hitbox_manager_.GetHitboxFor(event.target_id)) {
      target_pos = hitbox->position;
    } else {
      LOG_ERR(
          "[CombatController] Animation target creature %d not found in "
          "hitboxes!",
          event.target_id);
  ...
  if (auto hitbox = hitbox_manager_.GetHitboxFor(event.source_id)) {
    ...
    LOG_ERR(
        "[CombatController] Animation source creature %d not found in "
        "hitboxes!",
        event.source_id);
    anim.start_pos = {0, 0};
  }
