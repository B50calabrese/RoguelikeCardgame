...
RuleResult CreatureAttackAction::Validate(const state::GameState& state) const {
  ...
  if (target_.type == Target::Type::kCreature) {
  ...
  } else if (target_.type != Target::Type::kPlayer &&
...
void CreatureAttackAction::Apply(state::GameState& state) const {
  ...
  LOG_INFO(
      "[CreatureAttackAction] %s (ID:%d) attacking target (ID:%d, Type:%d)",
      attacker->data->name.c_str(), attacker_id_, target_.id,
      (int)target_.type);
  ...
  EffectResolver::Get().QueueAction(std::make_shared<DealDamageAction>(
      attacker_id_, target_, attacker->current_power));
  ...
  if (target_.type == Target::Type::kCreature) {
    ...
    if (target_inst) {
      ...
      attacker_target.type = Target::Type::kCreature;
      attacker_target.id = attacker_id_;
      EffectResolver::Get().QueueAction(std::make_shared<DealDamageAction>(
          target_inst->instance_id, attacker_target,
          target_inst->current_power));
    }
  }

  // Add a visual blocker for the animation
  // The Scene will be responsible for handling the animation and removing this
  // blocker
  VisualBlocker::Get().AddBlocker("CreatureAttack_" +
