...
void DealDamageAction::Apply(GameState& state) const {
  ...
  if (target_.type == Target::Type::kPlayer) {
    ...
    LOG_INFO("[EffectResolver] Player takes %d damage. Health: %d", amount_,
             state.player->health);
    publish_events(GameEventType::PlayerTookDamage);
  } else if (target_.type == Target::Type::kEnemy) {
    state.enemy->health -= amount_;
    LOG_INFO("[EffectResolver] Enemy takes %d damage. Health: %d", amount_,
             state.enemy->health);
    publish_events(GameEventType::PlayerTookDamage);
    ...
    if (target_inst && target_inst->location == CardLocation::Board) {
      ...
      LOG_INFO("[EffectResolver] Creature %s takes %d damage. Health: %d",
      ...
      if (target_inst->current_health <= 0) {
        EffectResolver::Get().QueueAction(
            std::make_shared<KillCreatureAction>(target_inst->instance_id));
      }
    }
  }
}
