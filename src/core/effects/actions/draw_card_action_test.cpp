...
TEST_F(DrawCardActionTest, DrawMultiple) {
  state->player->deck.push_back(
      std::make_unique<CardInstance>(card_data.get(), 101, 0));
  state->player->deck.push_back(
      std::make_unique<CardInstance>(card_data.get(), 102, 0));
  state->player->deck.push_back(
      std::make_unique<CardInstance>(card_data.get(), 103, 0));
