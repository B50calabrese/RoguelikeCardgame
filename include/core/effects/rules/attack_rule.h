...
/**
 * @brief Rule enforcing attack constraints (turn, exhausted, summoning
 * sickness).
 */
class AttackRule : public IRule {
 public:
  RuleResult Validate(const state::CombatState& state,
    ...
    auto attack_action =
        std::dynamic_pointer_cast<actions::CreatureAttackAction>(action);
    if (!attack_action) return {true, "Not an attack action", false};

    CardInstance* attacker =
        state.FindCardInstance(attack_action->attacker_id());
    ...
    if (!attacker->can_attack) {
      return {false, "Creature cannot attack this turn (Summoning Sickness?)",
              false};
    }
    ...
    } else if (attack_action->target().type == Target::Type::kPlayer) {
      if (attacker->owner_id == state.player->id) {
        return {false, "Cannot attack yourself", false};
      }
    } else if (attack_action->target().type == Target::Type::kEnemy) {
