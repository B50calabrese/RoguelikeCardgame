...
class TargetingRule : public IRule {
 public:
  RuleResult Validate(const state::CombatState& state,
    ...
    // If it's a spell, check if there are legal targets for EVERY effect that
    // requires one.
    if (inst->data->type == CardType::Spell) {
      for (const auto& effect_def : inst->data->effects) {
        ...
        if (effect_def.filter.IsValid(
                state, player_id, {Target::Type::kPlayer, state.player->id}))
          found_legal_target = true;
        if (!found_legal_target &&
            effect_def.filter.IsValid(state, player_id,
                                      {Target::Type::kEnemy, state.enemy->id}))
          found_legal_target = true;
        ...
        if (!found_legal_target) {
          for (const auto& c : state.player->board) {
            if (effect_def.filter.IsValid(
                    state, player_id,
                    {Target::Type::kCreature, c->instance_id})) {
              found_legal_target = true;
              break;
            }
          }
