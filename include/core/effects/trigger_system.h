  ...
  void HandleEvent(state::GameState& state, const GameEvent& event) {
    ...
    switch (event.type) {
      case GameEventType::CardPlayed:
        trigger_to_check = Trigger::OnPlay;
        break;
      case GameEventType::CreatureDied:
        trigger_to_check = Trigger::OnDeath;
        break;
      case GameEventType::TurnStarted:
        trigger_to_check = Trigger::AtStartOfTurn;
        break;
      case GameEventType::TurnEnded:
        trigger_to_check = Trigger::AtEndOfTurn;
        break;
      default:
        return;  // Not a triggered event type
    }
    ...
    if (source_inst) {
      for (const auto& effect_def : source_inst->data->effects) {
        if (effect_def.trigger == trigger_to_check) {
          // If the effect requires targets, this simple system might need human
          // input or AI For now, we only automatically trigger non-targeted
          // effects or effects where targets are provided.
          if (effect_def.filter.is_required) continue;

          auto effect =
              EffectRegistry::Get().CreateEffect(effect_def.effect_type);
          if (effect) {
            auto actions = effect->GenerateActions(source_inst->instance_id, {},
                                                   effect_def.params);
    ...
    // Also check OTHER cards on the board for triggers (e.g. "Whenever a
    // creature dies...")
    auto check_others =
        [&](const std::vector<std::unique_ptr<CardInstance>>& board) {
          for (const auto& inst : board) {
            if (source_inst && inst->instance_id == source_inst->instance_id)
              continue;
            for (const auto& effect_def : inst->data->effects) {
              if (effect_def.trigger == trigger_to_check &&
                ...
                auto effect =
                    EffectRegistry::Get().CreateEffect(effect_def.effect_type);
                if (effect) {
                  auto actions = effect->GenerateActions(inst->instance_id, {},
                                                         effect_def.params);
                  QueueActions(actions);
                }
              }
