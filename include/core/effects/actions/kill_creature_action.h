 ...
 public:
  ...
  void Apply(state::GameState& state) const override {
    auto find_and_kill = [this](PlayerState& p, int id) -> bool {
      ...
      if (it != p.board.end()) {
        LOG_INFO("[EffectResolver] Creature %s dies.",
                 (*it)->data->name.c_str());
        ...
        // Note: In a full implementation, we would trigger an "OnDeath" event
        // here that could queue further actions for cards on the board that
        // care about death.
        return true;
      }
      return false;
