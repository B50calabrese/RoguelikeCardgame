...
class ResolveSpellAction : public ActionBase {
 public:
  ResolveSpellAction(int player_id, int card_instance_id,
  ...
  void Apply(state::GameState& state) const override {
    ...
    // NOTE: In a full UI implementation, we'd wait for user targeting if
    // targets_ is empty but the spell requires targets. For now, we assume
    // targets_ contains the needed targets or we use the first available legal
    // target if needed (placeholder logic).
    ...
    for (const auto& effect_def : inst->data->effects) {
      ...
      if (effect) {
        ...
        // TODO: Improve targeting logic. For now, we pass the targets_ provided
        // to PlayCardAction.
        auto actions = effect->GenerateActions(card_instance_id_, targets_,
                                               effect_def.params);
    ...
    EffectResolver::Get().QueueAction(
        std::make_shared<MoveToGraveyardAction>(card_instance_id_));
  }
