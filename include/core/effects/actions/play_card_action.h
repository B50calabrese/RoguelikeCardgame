...
class PlayCardAction : public ActionBase {
 public:
  PlayCardAction(int player_id, int card_instance_id,
  ...
  void Apply(state::CombatState& state) const override {
    ...
    auto it = std::find_if(p.hand.begin(), p.hand.end(),
                           [id = card_instance_id_](const auto& c) {
    ...
    if (it != p.hand.end()) {
      ...
      EventBus::Get().Publish(state,
                              {GameEventType::CardPlayed, player_id_, card_id});
    }
  }

  std::string name() const override { return "PlayCard"; }

  int actor_id() const override { return player_id_; }
